/**
 * Time helpers for a product where every displayed hour belongs to the trail, not the reader:
 * everything is computed in epoch seconds and rendered once with the trail's own UTC offset,
 * which Open-Meteo returns alongside the forecast.
 *
 * Nothing here touches the host machine's timezone — `Date` is used only as an epoch→field
 * decoder via its `getUTC*` accessors.
 */
...
/**
 * An instant as ISO 8601 in the trail's local time, offset included. The offset is load-bearing:
 * `…T11:20:00+01:00` survives a round trip, `…T11:20:00` is a different moment per parser.
 */
...
/**
 * A sensible default start when the caller named none: the next 07:00 local to the trail that
 * has not already passed. Defaulting to *now* would answer a question nobody asked.
 */
...
/**
 * Index of the hourly slot covering `epochS` — the last entry at or before it. Nearest-hour
 * rather than interpolated: the upstream model resolves to the hour, and smoothing between two
 * of its values would invent precision the forecast does not have. Null outside the window.
 */
