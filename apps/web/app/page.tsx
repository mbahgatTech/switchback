...
/**
 * The front page is the list of hikes nearest the reader.
 *
 * It used to be a pitch — a headline, a sample elevation section, a legend of the four
 * plates — and every one of those was a page about the product rather than a page of the
 * product. The single sample forecast in particular read as though the forecast were the
 * only thing here, which was the fair reading of a page that showed one feature and
 * described three.
 *
 * So it shows the thing instead. Somewhere between five and twelve real trails, ordered by
 * how far away they are, each a link straight to its own page. Nothing above the list but a
 * line saying where "near" is measured from and how we know, because that sentence is the
 * only thing standing between an honest list and a list that is quietly about a city the
 * reader is not in.
 *
...
export const metadata: Metadata = {
  title: `${BRAND.name} — trails near you`,
...
export default async function HomePage() {
  ...
  return (
    <div className="min-h-dvh bg-canvas text-ink">
      <header className="mx-auto flex max-w-[880px] flex-wrap items-center justify-between gap-md px-xl py-lg">
        <Wordmark large home={false} />

        <span className="flex items-center gap-lg">
          <SiteNav />
          {/*
           * Signing in is one link to the one page that does it, rather than the pair of
           * provider buttons that used to sit halfway down this page. A front page carrying
           * its own OAuth forms is a front page that has decided signing in is the point;
           * everything here works signed out, so it is a link in the margin.
           */}
          {viewer === null ? (
            <Link href="/signin" className="collar rounded-hair hover:text-ink">
              Sign in
            </Link>
          ) : null}
        </span>
