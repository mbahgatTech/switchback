/**
 * Turning OSM elements into one ordered line per trail: a route relation is an *unordered
 * set* of member ways, so this chains by shared endpoints, reverses what needs reversing, and
 * bridges the small gaps that are mapping noise but not the large ones that are two trails.
 *
 * Coordinate equality is a safe join test because `out geom` returns each shared node's
 * position from the same node record; the epsilon guards float formatting, not real geometry.
 */
...
/**
 * Gaps up to this are bridged with a straight line: roughly the width of the features that
 * cause them — a road crossing, an untagged bridge — and far too short to connect two trails
 * that merely end near each other.
 */
...
function keyOf([lng, lat]: LngLat): string {
  // Quantising to 1e-7 makes the lookup an exact-match hash rather than a scan, and the
  // quantum is below the precision OSM stores, so it cannot merge distinct nodes.
...
/**
 * Chain ways into maximal contiguous lines. Greedy rather than an Eulerian path solver: a real
 * trail is a path, not a network, so greedy finds the same answer, and where a relation *is*
 * branchy it produces the main line plus the spur, which `pickPrimary` resolves. A path solver
 * would produce one line that hikes the spur and doubles back.
 */
...
/**
 * Join lines whose endpoints are within `toleranceM`, repeatedly, until nothing more joins.
 * Runs after `chainWays` because a shared node is always the better join.
 *
 * O(n²) per pass over segment endpoints, where n is the number of *disconnected runs* in one
 * relation — single digits in practice, and the length filter discards anything worse.
 */
...
/**
 * The line a relation is actually about: longest wins. A no-op on a well-mapped relation; on
 * one carrying an alternate start or an approach spur it picks the route the name refers to.
 */
...
/**
 * Drop consecutive duplicate vertices. A relation listing the same way twice — common on
 * out-and-backs mapped as one relation — produces zero-length segments, which contribute
 * nothing to length and divide by zero in the grade calculation.
 */
...
/**
 * Assemble every trail present in one Overpass response. Relations first, and the ways they
 * consume are excluded from standalone consideration — otherwise a named way inside the Tour
 * du Mont Blanc becomes its own trail alongside it and the map shows the path twice.
 */
export function assembleTrails(
  ...
  /**
   * The discriminator for relation fragments. `claimedWayIds` is narrower — only the ways on
   * a relation's *primary* chain — because a relation whose members form two disjoint lines
   * has a real second line worth keeping. But OSM tags a long route's constituent ways with
   * the route's own name, so "Huckleberry Trail" arrives as a 59 km relation *and* as a 40 km
   * way that fell off the chain, and the reader sees one trail at two lengths.
   *
   * **Both conditions are required.** A way a relation claims *and* names as one we already
   * emitted is a fragment. A way a relation claims but names differently — the Mist Trail
   * inside the John Muir Trail — is a real trail, and dropping it makes it unsearchable.
   */
  ...
  for (const relation of relations) {
    ...
    for (const member of relation.members ?? []) {
      ...
      // Relation members carry inline geometry from `out geom`; fall back to the way element
      // when the member entry is bare, which happens on very large relations.
   ...
   * `highway=path` ways comes out as one trail. Name is the only signal available, and the
   * gap tolerance is what stops two unrelated "Ridge Trail"s in one tile merging.
   */
  ...
  for (const way of waysById.values()) {
    ...
    if (!name) continue;
    // A relation member wearing that relation's name is a fragment of it. Both halves matter;
    // either alone drops real trails.
  ...
  for (const [name, group] of byName) {
    for (const line of bridgeGaps(chainWays(group), gapToleranceM)) {
      ...
      // The lowest way id is stable across ingests, which matters because it is half of the
      // (osmType, osmId) unique key that makes re-ingesting a tile an update, not a duplicate.
...
/**
 * Trail names reduced to something comparable, since casing and spacing drift between a
 * relation and the ways beneath it. Nothing stronger than case and whitespace is folded:
...
/**
 * Relation tags win; member-way tags fill the gaps. Physical attributes — surface, sac_scale,
 * access — live on the ways because that is where they vary. The majority value across
 * members, not the first: one stretch of boardwalk must not relabel 12 km as `surface=wood`.
 */
function mergeTags(
