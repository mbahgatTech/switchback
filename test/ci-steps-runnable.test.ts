...
/**
 * A runner invoked with `-e`/`--eval`, however its argument is written.
 *
 * Deliberately looser than the harvest below, which only understands a single quoted argument.
 * A heredoc or an unquoted eval would slip past the strict form and be silently untested — the
 * exact shape of miss this file exists to prevent — so the two counts are compared and a
 * mismatch fails rather than passes quietly.
 */
const INLINE_EVAL_SITE = /(?:^|[|&;\s])(?:\S*[/\\])?(?:node|tsx|ts-node)\s+(?:-e|--eval)(?:\s|$)/g;

/** How many inline evals exist, by the loose reading. */
function inlineEvalSites(): number {
  return workflowFiles().reduce(
    (total, workflow) =>
      total +
      steps(workflow).reduce((n, { run }) => n + [...run.matchAll(INLINE_EVAL_SITE)].length, 0),
    0,
  );
}
...
describe('every inline eval a workflow runs', () => {
  ...
  it('harvests all of them, in a form it can run', () => {
    // A harvest that silently matched nothing would make every case below vacuous, and one that
    // matched only some would leave the unmatched ones as untested as the probe that started this.
    expect(evals.length).toBeGreaterThan(0);
    expect(evals.length, 'an inline eval exists that the harvest cannot run').toBe(
      inlineEvalSites(),
    );
  });
